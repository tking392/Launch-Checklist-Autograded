...
function addDestinationInfo(document, name, diameter, star, distance, moons, imageUrl) {
 ...
 function formSubmission(document, list, pilot, copilot, fuelLevel, cargoLevel) {
        ...
        if (validateInput(pilot) === 'Empty' || validateInput(copilot) === 'Empty' || validateInput(fuelLevel) === 'Empty' || validateInput(cargoLevel) === 'Empty' ) {
            return alert('All fields required. Please enter all required information.');
        }
        // checks to make sure pilot and copilot are strings and fuel and cargo are numbers
        else if (validateInput(pilot) === 'Is a Number' || validateInput(copilot) === 'Is a Number') {
           return alert('Number entered in Pilot and Co-pilot fields. Please enter a name.');
        }  
        else if (validateInput(fuelLevel) === 'Not a Number' || validateInput(cargoLevel) === 'Not a Number') {
           return  alert('Invalid entry. Please enter a numeric value for Fuel and Cargo.');
        } 
        ...
        //update to compare both fuelLevel and CargoMass on each check
        ...
        // if both cargo and fuel are not at the correct levels to launch
        else if (Number(cargoLevel) > 10000  && Number(fuelLevel) < 10000) {
            faultyItems.style.visibility = 'visible';
            fuelStatus.innerHTML = 'Fuel level too low for launch';
            cargoStatus.innerHTML = 'Cargo mass too heavy for launch';
